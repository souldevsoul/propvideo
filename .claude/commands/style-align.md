...
You are helping a developer align a page or component to PropVideo's design system. This is for cases where the functionality works, but the visual style doesn't match the brand.
...
Fix visual/style inconsistencies to match PropVideo's design system.
...
## Step 2: PropVideo Design System Reference

### Brand Colors (ONLY USE THESE!)

**Primary:**
- 🔵 Teal/Sky: `teal-500`, `sky-500`, `sky-600`
...
shadow-glow-sky   // Sky blue glow effect
```
...
**After:**
```tsx
<button className="bg-gradient-to-r from-teal-500 to-sky-600 hover:shadow-glow-teal">
  Create Video
</button>
```
...
**Before:**
```tsx
<button className="bg-teal-500 text-white px-6 py-3">
  Button
</button>
```
...
**After:**
```tsx
<button className="bg-gradient-to-r from-teal-500 to-sky-600 text-white px-6 py-3 rounded-xl shadow-soft-md hover:shadow-glow-teal hover:scale-105 transition-all duration-300">
  Button
</button>
```
...
3. **Fix colors first** - Replace yellow/orange/emerald with teal/sky/indigo
...
**Checklist:**
- [ ] Colors match PropVideo palette (teal/sky/indigo)
...
<div className="border-4 border-black shadow-[0_8px_0_0_#000] rounded p-6">
  ...
  <div className="text-4xl font-black text-yellow-400 mb-4">$49</div>
...
### After (PropVideo style - soft, teal/sky)
```tsx
<div className="bg-white rounded-3xl shadow-soft-xl p-10 hover:shadow-glow-teal transition-all duration-300 border border-slate-100">
